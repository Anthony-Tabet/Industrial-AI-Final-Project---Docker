...
def perform_object_detection(video_path):
    model = YOLO(r'C:\Users\Antho\OneDrive\Desktop\Industrial AI Final Project - Docker\Road_Pothole_detection\model\best.pt')
    model.to("cuda")
    current_time = time.strftime("%Y%m%d_%H%M%S")  # Get current timestamp
    name_dir = "Pothole_Detection_"+ current_time
    model.predict(video_path,  conf=0.25, name=name_dir, save=True)
    return name_dir

# Route to upload video and display the processed video
@app.route('/RoadPotholeDetection', methods=['GET', 'POST'])
def upload_video():
